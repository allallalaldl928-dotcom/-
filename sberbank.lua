-- SBERBANK HUB [ФИНАЛЬНЫЙ ФИКС ФЛАЯ И ФЛИНГА]
...
-- 1. БЕЗОПАСНЫЙ ФЛИНГ (Без улета за карту)
local flingActive = false
AddButton("Fling (Безопасный)", function(v) flingActive = v end)
...
RunService.Heartbeat:Connect(function()
    if flingActive then
        ...
        if rootPart then
            rootPart.AssemblyLinearVelocity = Vector3.new(0, 30, 0) + (Camera.CFrame.LookVector * 20)
            rootPart.CFrame = rootPart.CFrame * CFrame.Angles(0, math.rad(50), 0)
        end
    end
end)

-- 2. РАБОЧИЙ ФЛАЙ (С управлением через WASD)
local flying = false
local flySpeed = 50
local bv, bg
...
print("Sberbank Hub [Fixed] успешно запущен!")
